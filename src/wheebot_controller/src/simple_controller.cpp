#include "wheebot_controller/simple_controller.hpp"
...
SimpleController::SimpleController(const std::string & name)
    : Node(name)
{
    declare_parameter("wheel_radius", 0.7);
    declare_parameter("wheel_separation", 0.6);
    ...
    vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>("/wheebot_controller/cmd_vel", 10, std::bind(&SimpleController::velCallback, this, _1));
