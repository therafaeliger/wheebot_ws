...
public:
    ...
    {
        declare_parameter<int>("simple_int_param", 34);
        declare_parameter<std::string>("simple_string_param", "Radya");
...
private:
    ...
    rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter> & parameters)
    {
        ...
        for(const auto& param : parameters)
        {
            ...
            {
                RCLCPP_INFO_STREAM(get_logger(), "Parameter 'simple_int_param' changed to: " << param.as_int());
                result.successful = true;
            }

            if(param.get_name() == "simple_string_param" && param.get_type() == rclcpp::ParameterType::PARAMETER_STRING)
            {
                RCLCPP_INFO_STREAM(get_logger(), "Parameter 'simple_string_param' changed to: " << param.as_string());
                result.successful = true;
            }
        }
